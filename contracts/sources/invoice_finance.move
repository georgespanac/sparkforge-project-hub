module invoice_finance::invoice_finance {
    ...
    public fun accept_finance<CoinType>(
        ...
        let (to_issuer, change) = coin::split(&mut payment, required);
    ...
    public fun confirm_payment<CoinType>(
        ...
        let coin_to_financier = coin::from_balance<CoinType>(to_financier, ctx);
        ...
        if (balance::value(&remainder) > 0) {
            let back = coin::from_balance<CoinType>(remainder, ctx);
            transfer::public_transfer(back, depositor);
        };
        ...
        // Extract remaining funds and delete escrow
        let SettlementEscrow { id, funds: leftover, .. } = escrow;
        // If something left (shouldn't), return to oracle
        if (balance::value(&leftover) > 0) {
            let c = coin::from_balance<CoinType>(leftover, ctx);
            transfer::public_transfer(c, registry.oracle);
        };
        object::delete(id);
    }
